...
输出语言：{{language}}`,
  ...
  prompt({
    id: 'card-forge.game-card.generate', category: 'card-forge', name: '卡牌工坊卡面生成',
    description: '根据数据卡和附加要求生成游戏卡面元数据；必需槽位由服务端提供，内容安全与输出 Schema 由不可编辑保护层兜底。', active: true, managementMode: 'overlay',
    source: 'lib/game-card/config.ts',
    defaultBody: `你是一位资深的卡牌游戏设计师，精通各种卡牌游戏（如万智牌、游戏王、炉石传说、影之诗等）的卡面设计原则。
...
6. traits 数组至少包含 1 个关键词，通常 2-5 个`,
    slots: [
      required('sourceCardJson', '服务端提供的原始数据卡正文', '{"cardName":"示例角色"}'),
      required('customInstructions', '用户附加要求；未提供时由服务端填入明确占位文本', '突出角色的控制能力'),
    ],
    previewVariables: {
      sourceCardJson: '{"cardName":"示例角色","type":"magical-girl"}',
      customInstructions: '突出暗属性与控制能力',
    },
  }),
  prompt({
