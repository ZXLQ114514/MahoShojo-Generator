...
type Tab = 'cards' | 'reports' | 'users' | 'settings';
...
export function AdminPage() {
  ...
  }, [authLoading, isAuthenticated, loadCards, loadReports, loadSettings, loadUsers, tab]);
  ...
  const title = useMemo(() => tab === 'cards' ? '角色卡审核' : tab === 'reports' ? '战报管理' : tab === 'users' ? '用户管理' : '系统设置', [tab]);
    ...
    <main className="magic-background-white min-h-screen py-6">
      <div className="container">
        ...
        <div className="mb-4 flex gap-2 border-b border-gray-200">
          {([['cards', '角色卡审核'], ['reports', '战报管理'], ['users', '用户管理'], ['settings', '系统设置']] as const).map(([value, label]) => (
