  ...
  return (
    <main className="magic-background-white min-h-screen py-8">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!loading && records.length > 0 ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{records.map((record) => <button key={record.id} type="button" onClick={() => void openReport(record.id)} className="group rounded-xl border border-slate-700 bg-slate-900/90 p-4 text-left text-slate-200 shadow-sm transition hover:-translate-y-0.5 hover:border-pink-300/40 hover:bg-slate-800/90 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-300/60 sm:p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="inline-flex rounded-full border border-pink-300/20 bg-pink-400/10 px-2 py-1 text-xs font-medium text-pink-200">生成模式：{modeLabels[record.mode || ''] || record.mode || '未知模式'}</span><h2 className="mt-3 line-clamp-2 text-lg font-semibold text-white">{record.headline || '未命名战报'}</h2></div><span className="text-xs text-slate-400 transition group-hover:text-slate-200">查看</span></div>{record.scenarioTitle ? <p className="mt-2 text-xs text-slate-400">情景：{record.scenarioTitle}</p> : null}{record.note ? <p className="mt-2 line-clamp-2 text-xs leading-5 text-pink-200/90">备注：{record.note}</p> : null}<p className="mt-3 line-clamp-3 whitespace-pre-wrap text-sm leading-6 text-slate-300">{record.excerpt}</p><div className="mt-4 flex flex-wrap gap-x-3 gap-y-1 border-t border-slate-700 pt-3 text-xs text-slate-400"><span>胜者：{record.winner || '未解析'}</span><span>上传人：{record.username || '未知用户'}</span><span>公开于 {formatDate(record.publicSince)}</span></div></button>)}</div> : null}
